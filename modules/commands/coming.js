...
module.exports.run = async (MAIN, message, raids, discord) => {
  ...
  function count_members(){
    ...
    MAIN.pdb.query(`SELECT * FROM lobby_members WHERE gym_id = ?`, [raids.gym_id], function (error, lobbys, fields) {
      ...
      return MAIN.channels.get(channel).send('<@'+member+'> is **on the way**! There are:```\n'
...
function insert_user(count){
  ...
  MAIN.pdb.query(`INSERT INTO lobby_members (gym_id, user_id, count, arrived) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE count = ?, arrived = ?`, [raids.gym_id, member,count,'coming',count, 'coming'], function (error, lobby, fields) {
    if(error){ console.error(error); }
  });
  count_members();
}
