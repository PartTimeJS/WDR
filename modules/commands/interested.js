...
module.exports.run = async (MAIN, message, raids, count) => {
  ...
  let nickname = '', lobby_users = '<@&'+raids.role_id+'>';
  ...
  function count_members(){
    ...
    MAIN.pdb.query(`SELECT * FROM lobby_members WHERE gym_id = ?`, [raids.gym_id], function (error, lobbys, fields) {
      ...
      return MAIN.channels.get(channel).send('<@'+member+'> has shown interest with '
...
function update_status(){
  MAIN.pdb.query(`UPDATE lobby_members SET arrived = ? WHERE gym_id = ? && user_id = ?`, ['interested',raids.gym_id,member], function (error, lobby, fields) {
    if(error){ console.error(error); }
  });
}
...
function insert_user(count){
  if (count === 0) {
    ...
    MAIN.pdb.query(`INSERT INTO lobby_members (gym_id, user_id, count, arrived) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE count = ?, arrived = ?`, [raids.gym_id, member,count,'interested',count, 'interested'], function (error, lobby, fields) {
      if(error){ console.error(error); }
    });
  }
  count_members();
