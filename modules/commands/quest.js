...
async function subscription_time(MAIN, message, nickname, prefix, discord){
  ...
  MAIN.pdb.query(`SELECT * FROM users WHERE user_id = ? AND discord_id = ?`, [message.author.id, discord.id], async function (error, user, fields) {
    ...
    switch (sub) {
      case 'cancel':
        // CREATE THE EMBED
        let cancelled = new Discord.RichEmbed()
          .setAuthor(nickname, message.author.displayAvatarURL)
      ...
      case 'time':
        // CREATE THE EMBED
        let timed_out = new Discord.RichEmbed()
          .setAuthor(nickname, message.author.displayAvatarURL)
...
async function subscription_create(MAIN, message, nickname, prefix, discord){
  ...
  MAIN.pdb.query(`SELECT * FROM users WHERE user_id = ? AND discord_id = ?`, [message.author.id, discord.id], async function (error, user, fields) {
    ...
    // RETRIEVE QUEST NAME FROM USER
    let sub = await sub_collector(MAIN, 'Name', nickname, message, user[0].quests, 'Type any Pokémon name, or choose from the list. A ✔ denotes you are already subscribed to that Reward. Names are not case-sensitive.', undefined);
    switch (sub) {
      case 'cancel':
        // CREATE THE EMBED
        let cancelled = new Discord.RichEmbed()
      ...
      collector.on('collect', message => {
        switch(true){
          ...
          case type.indexOf('Remove')>=0:
            ...
            let valid_pokemon = MAIN.Pokemon_ID_Search(search_pokemon);
            if(valid_pokemon){
