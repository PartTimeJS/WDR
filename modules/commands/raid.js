...
async function subscription_create(MAIN, message, member, prefix, advanced, available_gyms, discord, gym_collection){
  ...
  do {
    ...
    if(sub.gym == 'cancel'){ return subscription_cancel(MAIN, member, message, prefix, available_gyms, discord, gym_collection); }
      ...
      if(sub.gym == 'All'){ sub.gym = 'All'; got_name = true; }
        ...
        let user_choice = await match_collector(MAIN, 'Multiple', member, message, undefined, 'Type the number of the Correct Gym.', sub, available_gyms, discord, gym_collection);
        if(sub.gym == 'cancel'){ return subscription_cancel(MAIN, member, message, prefix, available_gyms, discord, gym_collection); }
        else if(sub.gym == 'time'){ return subscription_timedout(MAIN, member, message, prefix, available_gyms, discord, gym_collection); }
  ...
  if(sub.boss == 'All' || sub.boss == 'Egg'){
    ...
    sub.min_lvl = await sub_collector(MAIN,'Minimum Level',member,message, sub,'Please respond with a value of 1 through 5 or type \'All\'. Type \'Cancel\' to Stop.', sub, available_gyms, discord, gym_collection);
    ...
    // RETRIEVE MIN LEVEL FROM USER
    if(sub.min_lvl == '5' || sub.min_lvl == 'All'){ sub.max_lvl = 'All'; }
    else{
    ...
    if(!user[0].raids){
      ...
      if(!raid.subscriptions[0]){ raid.subscriptions.push(sub); }
        ...
        raid.subscriptions.forEach((subscription,index) => {
          subscription.form = subscription.form ? subscription.form : sub.pokemon.pokemon_id ? MAIN.masterfile.pokemon[sub.pokemon.pokemon_id].default_form : 0;
...
function sub_collector(MAIN, type, member, message, object, requirements, sub, available_gyms, discord, gym_collection){
    ...
    switch(type){
      ...
      case 'Remove':
        ...
        await MAIN.asyncForEach(sub.subscriptions, async (sub,index) => {
          ...
          if(id && sub.pokemon){
            remove_locale = await MAIN.Get_Locale(MAIN, {pokemon_id: sub.pokemon.pokemon_id, form: sub.form ? sub.form : id.form}, discord);
      ...
      collector.on('collect', message => {
        switch(true){
          ...
          case type.indexOf('Remove') >= 0:
            if(message.content > 0 && message.content <= sub.subscriptions.length){ collector.stop(message.content); }
...
function initiate_collector(MAIN, source, message, msg, member, prefix, available_gyms, discord, gym_collection){
  ...
  collector.on('end', (collected,reason) => {
    ...
    switch(reason){
      ...
      case 'add': subscription_create(MAIN, message, member, prefix, false, available_gyms, discord, gym_collection); break;
      case 'remove': subscription_remove(MAIN, message, member, prefix, available_gyms, discord, gym_collection); break;
      case 'view': subscription_view(MAIN, message, member, prefix, available_gyms, discord, gym_collection); break;
      case 'resume':
      case 'pause': subscription_status(MAIN, message, member, reason, prefix, available_gyms, discord, gym_collection); break;
      default:
        if(source == 'start'){
