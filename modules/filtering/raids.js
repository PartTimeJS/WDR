...
module.exports.run = async (MAIN, raid, main_area, sub_area, embed_area, server, timezone, role_id) => {
    ...
    // FILTER FOR EGG LEVEL
      ...
      if(geofences.indexOf(server.name) >= 0 || geofences.indexOf(main_area) >= 0 || geofences.indexOf(sub_area) >= 0){
        ...
        if(filter.Ex_Eligible_Only == undefined || filter.Ex_Eligible_Only != true){
          ...
          await raid_lobbies(MAIN, raid, boss_name, channel, embed_area, timezone, server);
        ...
        if(MAIN.debug.Raids == 'ENABLED' && MAIN.debug.Feed == 'ENABLED'){ console.info('[FILTERING] ['+MAIN.Bot_Time(null,'stamp')+'] [raids.js] Raid Did Not Pass Channel Geofences for '+raid_channel[0]+'. Expected: '+raid_channel[1].geofences+' Saw: '+server.name+'|'+main_area+'|'+sub_area); }
      }
    } else{
  ...
  function raid_lobbies(MAIN, raid, boss_name, channel, embed_area, timezone, server){
    return new Promise(resolve => {
      ...
      if(raid.level >= server.min_raid_lobbies){
        ...
        [raid.gym_id, raid.gym_name, JSON.stringify(raid), server.id, channel.id, embed_area, boss_name, end_time, raid.end, JSON.stringify(raid)], function (error, record, fields) {
