...
module.exports = async (WDR, message, prefix, server) => {
  ...
  let geofence = await WDR.Geofences.get(server.geojson_file);
  ...
  message.channel.send(requestAction).catch(console.error).then(msg => {
    ...
    collector.on("collect", message => {
      switch (message.content.toLowerCase()) {
        ...
        case "cancel":
          ...
          break;
        default:
          message.reply("`" + message.content + "` is not a valid option.").then(m => m.delete({
            timeout: 5000
          })).catch(console.error);
      }
    });
    ...
    collector.on("end", (collected, reason) => {
      ...
      switch (reason) {
        case "add":
          subscription_create(WDR, message, nickname, prefix, area_array, server);
          break;
        case "remove":
          subscription_remove(WDR, message, nickname, prefix, area_array, server);
          break;
        case "view":
          subscription_view(WDR, message, nickname, prefix, area_array, server);
          break;
        default:
...
// AREA VIEW FUNCTION
async function subscription_view(WDR, message, nickname, prefix, area_array, server) {
  WDR.wdrDB.query(`SELECT * FROM users WHERE user_id = ? AND discord_id = ?`, [message.author.id, server.id], function(error, user, fields) {
    if (!user || !user[0]) {
      return;
    }
...
async function subscription_create(WDR, message, nickname, prefix, area_array, server) {

  // PULL THE USER"S SUBSCRITIONS FROM THE USER TABLE
  WDR.wdrDB.query(`SELECT * FROM users WHERE user_id = ? AND discord_id = ?`, [message.author.id, server.id], async function(error, user, fields) {

    // RETRIEVE AREA NAME FROM USER
    let sub = await sub_collector(WDR, "Name", nickname, message, "Names are not case-sensitive. The Check denotes you are already subscribed to that Area.", user[0].geofence, area_array, server);
    ...
    WDR.wdrDB.query(`UPDATE users SET geofence = ? WHERE user_id = ? AND discord_id = ?`, [areas, message.author.id, server.id], function(error, user, fields) {
      if (error) {
...
async function subscription_remove(WDR, message, nickname, prefix, area_array, server) {

  // PULL THE USER"S SUBSCRITIONS FROM THE USER TABLE
  WDR.wdrDB.query(`SELECT * FROM users WHERE user_id = ? AND discord_id = ?`, [message.author.id, server.id], async function(error, user, fields) {

    // RETRIEVE AREA NAME FROM USER
    let sub = await sub_collector(WDR, "Remove", nickname, message, "Names are not case-sensitive. The Check denotes you are already subscribed to that Area.", user[0].geofence, area_array, server);
    if (sub.toLowerCase() == "cancel") {
      return message.reply("Subscription cancelled. Type `" + prefix + "area` to restart.").then(m => m.delete({
        timeout: 5000
      })).catch(console.error);
    ...
    if (sub == "all") {
      ...
      return message.reply("You are not subscribed to this Area.").then(m => m.delete(10000)).catch(console.error);
    } else {
...
function sub_collector(WDR, type, nickname, message, requirements, sub, area_array, server) {
        ...
        switch (true) {
          ...
          case type.indexOf("Remove") >= 0:
            ...
            for (let a = 0; a < area_array.length + 1; a++) {
              if (a == area_array.length) {
                message.reply("`" + message.content + "` doesn\"t appear to be a valid Area. Please check the spelling and try again.").then(m => m.delete({
                  timeout: 5000
                })).catch(console.error);
                break;
              } else if (message.content.toLowerCase() == area_array[a].toLowerCase()) {
                collector.stop(area_array[a]);
                break;
              }
            }
            break;
