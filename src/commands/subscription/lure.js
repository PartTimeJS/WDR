...
function subscription_status(WDR, message, nickname, reason, prefix, available_stops, discord, stop_collection) {
  WDR.wdrDB.query(`SELECT * FROM users WHERE user_id = ? AND discord_id = ?`, [message.author.id, discord.id], function(error, user, fields) {
    if (user[0].lure_status == 'ACTIVE' && reason == 'resume') {
      ...
      WDR.wdrDB.query('UPDATE users SET lure_status = ? WHERE user_id = ? AND discord_id = ?', [change, message.author.id, discord.id], function(error, user, fields) {
        if (error) {
...
async function subscription_view(WDR, message, nickname, prefix, available_stops, discord, stop_collection) {
  WDR.wdrDB.query(`SELECT * FROM users WHERE user_id = ? AND discord_id = ?`, [message.author.id, discord.id], function(error, user, fields) {
    ...
    if (!user[0].lure) {
      ...
      let lure = JSON.parse(user[0].lure),
        lure_levels = '';
      if (!lure.subscriptions[0]) {
        ...
        // CREATE THE EMBED
        let lure_subs = new WDR.DiscordJS.MessageEmbed()
          .setAuthor(nickname, message.author.displayAvatarURL)
...
async function subscription_create(WDR, message, nickname, prefix, advanced, available_stops, discord, stop_collection) {
  ...
  sub.type = await sub_collector(WDR, 'Type', nickname, message, undefined, 'Respond with \'All\'  or the Lure Type. Names are not case-sensitive.', sub, available_stops, discord, stop_collection);
  if (sub.type == 'cancel') {
    return subscription_cancel(WDR, nickname, message, prefix, available_stops, discord, stop_collection);
  ...
  WDR.wdrDB.query(`SELECT * FROM users WHERE user_id = ? AND discord_id = ?`, [message.author.id, discord.id], async function(error, user, fields) {
    ...
    if (!user[0].lure) {
      ...
      if (!lure.subscriptions[0]) {
        ...
        lure.subscriptions.forEach((subscription, index) => {
          ...
          } else if (index == lure.subscriptions.length - 1) {
            lure.subscriptions.push(sub);
          }
        });
      }
    ...
    WDR.wdrDB.query(`UPDATE users SET lure = ? WHERE user_id = ? AND discord_id = ?`, [new_subs, message.author.id, discord.id], function(error, user, fields) {
      if (error) {
        return message.reply('There has been an error, please contact an Admin to fix.').then(m => m.delete(10000)).catch(console.error);
      } else {
        let subscription_success = new WDR.DiscordJS.MessageEmbed().setColor('00ff00')
...
async function subscription_remove(WDR, message, nickname, prefix, available_stops, discord, stop_collection) {
  ...
  WDR.wdrDB.query(`SELECT * FROM users WHERE user_id = ? AND discord_id = ?`, [message.author.id, discord.id], async function(error, user, fields) {
    ...
    if (!user[0].lure) {
      ...
      let remove_id = await sub_collector(WDR, 'Remove', nickname, message, lure, 'Type the Number of the Subscription you want to remove.', undefined);
          ...
          case type.indexOf('Type') >= 0:
            switch (message.content.toLowerCase()) {
              ...
              case 'normal':
                ...
                break;
              case 'glacial':
                collector.stop('Glacial');
                break;
              case 'mossy':
                collector.stop('Mossy');
                break;
              case 'magnetic':
                collector.stop('Magnetic');
                break;
              default:
                message.reply('`' + message.content + '` doesn\'t appear to be a valid Lure Type. Please check the spelling and try again.').then(m => m.delete({
                  timeout: 5000
                })).catch(console.error);
            }
            break;
