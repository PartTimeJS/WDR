...
async function pokemon_view(WDR, message, nickname, target, prefix, discord) {
  ...
  message.reply('Searching... this may take a minute. Check your inbox if not in the channel.').then(m => m.delete({
    timeout: 5000
  })).catch(console.error);
  let search = '';
  if (target.duration) {
  ...
  WDR.scannerDB.query(query, [target.pokemon_id], function(error, stats, fields) {
    if (error) {
      console.error(error);
    }
    if (!stats) {
