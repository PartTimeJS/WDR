...
async function pokemon_view(WDR, message, nickname, pokemon, prefix, discord) {
  ...
  let pokemon_color = locale.color,
    level = 25;
  let pokemon_type = locale.type,
    weaknesses = locale.weaknesses;
  for (var atk = 15; atk >= 13; atk--) {
    for (var def = 15; def >= 13; def--) {
      for (var sta = 15; sta >= 13; sta--) {
        iv_percent = Math.round((atk + def + sta) / 45 * 100);
        result_string += atk + ',' + def + ',' + sta + ' ' + pvp.CalculateCP(WDR, pokemon_id, form_id, atk, def, sta, level) + ' CP ' + iv_percent + '%\n';
      }
    }
  }
  result_string += '```';
  ...
  if (discord.spam_channels.indexOf(message.channel.id) >= 0) {
    return WDR.Send_Embed(WDR, 'chart', 0, discord, role_id, chart_embed, message.channel.id);
  } else {
