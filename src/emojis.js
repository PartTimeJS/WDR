...
function LoadEmojis(bot, serverIDs) {
    ...
    for (var ga = 0; ga < guildArray.length; ga++) {
        let guild = bot.guilds.cache.get(guildArray[ga][0]);

        if (!guild) {
            console.log('Can\'t find guild with ID: ' + guildArray[ga][0].id + ' which is bot username: ' + bot.user.username + ' using a bot with token: ' + bot.token);
        }
