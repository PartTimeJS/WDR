      ...
      member.guild.fetchAuditLogs({
        ...
        if (userID == member.id) {
          auditLog.entries.map(log => {
            ...
            MessageEmbed = new Discord.MessageEmbed().setColor('ff0000')
          ...
          member.guild.fetchAuditLogs({
            ...
            if (userID == member.id && executor && auditLog.entries.map(u => u.executor.id) != member.id) {
              auditLog.entries.map(log => {
                ...
                MessageEmbed = new Discord.MessageEmbed().setColor('ffb200')
                  .setAuthor(uName + ' (' + member.id + ')', member.user.displayAvatarURL)
              ...
              MessageEmbed = new Discord.MessageEmbed().setColor('ffe500')
                .setAuthor(uName + ' (' + member.id + ')', member.user.displayAvatarURL)
