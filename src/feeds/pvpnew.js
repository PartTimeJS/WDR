module.exports = async (WDR, Sighting) => {
  ...
  WDR.PvP_Channels.forEach(async feed_channel => {
    ...
    if (defGeo || mainGeo || subGeo) {
      ...
      if (!channel.Filter.type) {
        console.error("[WDR " + WDR.Version + "] [" + WDR.Time(null, "log") + "] [feeds/pvp.js] Missing `type` variable in " + feed_channel[1].filter + ".");
        channel.Filter.type = "all";
      }
      ...
      if (cpRange && filterStatus) {
        ...
        Sighting[match.league].forEach(potential => {
        ...
        Sighting[match.league].forEach(async potential => {
          potential.types = await WDR.Get_Typing(WDR, {
          ...
          let rankMatch = potential.rank <= channel.Filter.min_pvp_rank;
          let cpMatch = potential.cp >= channel.Filter.min_cp_range;
          let typeMatch = channel.Filter.type == "all" ? true : potential.types.some(type => channel.Filter.type.includes(type));
          if (rankMatch && cpMatch && typeMatch) {
            ...
            filtered.value = potential.pvp_value;
            filtered.form_id = potential.form_id;
            match.alt_cps.push(filtered);
          }
        });

        if (match.possible_cps.length > 0) {
          console.log("ALT", match.alt_cps);
          console.log("POSSIBLE", match.possible_cps);
          ...
          match.pvp_data = "";
          match.possible_cps.forEach(pcp => {
            let pipe = " | "; // SPACING
            ...
            let string = Rank + " " + Name + " (" + Percent + ")\n" + Level + pipe + Cp + pipe + match.atk + "/" + match.def + "/" + match.sta;
            match.pvp_data += string + "\n";
          });
