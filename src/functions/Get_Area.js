  ...
  return new Promise(async resolve => {
    if (InsideGeojson.polygon(discord.geofence, [lon, lat])) {
      ...
      if (discord.geojson_file) {
        let geofence = await WDR.Geofences.get(discord.geojson_file);
        await geofence.features.forEach((geo, index) => {
          if (InsideGeojson.feature({
              features: [geo]
            }, [lon, lat]) != -1) {
            switch (geo.properties.sub_area) {
              case 'true':
                area.sub = geo.properties.name;
                break;
              default:
                area.main = geo.properties.name;
            }
          }
        });
      }
      ...
      if (area.main && !area.sub) {
        area.embed = area.main;
      }
      if (!area.sub && !area.main) {
        area.embed = discord.name;
      }
