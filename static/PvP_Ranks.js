...
function CalculateAllRanks(WDR) {
    return new Promise(async resolve => {
        ...
        for (var oPokemonID in pokemonObject) {
            if (pokemonObject[oPokemonID].attack) {
                CalculateTopRanks(WDR, oPokemonID, -1, 1500);
            }
            for (var oFormID in pokemonObject[oPokemonID].forms) {
                if (pokemonObject[oPokemonID].forms[oFormID].attack) {
                    CalculateTopRanks(WDR, oPokemonID, oFormID, 1500);
                }
            }
            greatbar.increment();
        }
    ...
    return await new Promise(async function(resolve) {
        ...
        WDR.wdrDB.query(
            ...
            function(error) {
                ...
                WDR.wdrDB.query('TRUNCATE ' + tableName + ';', async function(error) {
                    if (error) {
                        throw error;
                    }
                    return resolve(true);
...
async function InsertCurrentPokemon(WDR, tableName, pokemonID, formID, pokemon) {
    return await new Promise(async function(resolve) {
        ...
        WDR.wdrDB.query(sqlStatement, async function(error) {
            if (error) {
                throw error;
            }
            return resolve(true);
